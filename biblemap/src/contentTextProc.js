...
function getHistoryAdminStringObj( number ) {
    ...
    그 이후 예루살렘성을 정복하였으며 다윗 궁을 건축하기에 이른다" },
        ...
        { title : "BC 7C 앗수르 제국",
            text : "여호와께서 므낫세와 그 백성에게 이르셨으나 저희가 듣지 아니한 고로 <br>\
            여호와께서 앗수르 왕의 군대 장관들로 와서 치게 하시며 저희가 므낫세를 사로잡고 쇠사슬로 결박하여 바벨론으로 끌어간지라(역대하 33:10 - 11) <br>\
         <br> \
    ...
    그의 뜻은 이같지 아니하며 그의 마음의 생각도 이같지 아니하고 다만 그의 마음은 허다한 나라를 파괴하며 멸절하려 하는도다(이사야 10:5 - 7)<br> <br>\
    ...
    앗수르 왕 산헤립의 뒤를 이은 에살핫돈이 왕이 되어(열하 19:37)  시리아와 남유다 왕국을 포함하여 이집트까지 정복하게 되어 앗수르 최고의 전성기를 누리게 된다<br><br> - 미션와이드 - <br> "
        },
        { title : "BC 6C 바벨론 제국",
            text : "여호야김 시대에 바벨론의 왕 느부갓네살이 올라오매 여호야김이 삼 년간 섬기다가 돌아서 그를 배반하였더니  <br>\
            여호와께서 그의 종 선지자들을 통하여 하신 말씀과 같이 갈대아의 부대와 아람의 부대와 모압의 부대와 암몬 자손의 부대를 여호야김에게로 보내 유다를 쳐 멸하려 하시니( 열하 24:1-2 ) <br>\
         <br> \
            애굽 왕이 다시는 그 나라에서 나오지 못하였으니 이는 바벨론 왕이 애굽 강에서부터 유브라데 강까지 애굽 왕에게 속한 땅을 다 점령하였음이더라( 열하 24:7 ) <br>\
         <br> \
    ...
    바벨론 왕이 또 여호야긴의 숙부 맛다니야를 대신하여 왕으로 삼고 그의 이름을 고쳐 시드기야라 하였더라(열하 24:15-17) <br><br>\
...
function requestPoiContentAndShow( poiObj, tooltip2D, tooltip3D ) {
    var youtube = "";
    var localPoiObj = poiObj;
    var infoText = "";
    ...
    createPoiTooltip = function( popup, frameName, isFocus ){
        ...
        if (youtube != "") {
            if( frameName == "contentFrame2D" )
                labelText = youtubeRef2D + youtubeIcon + "</a>" + " " + poiText ;
            else if( frameName == "contentFrame3D" )
                labelText = youtubeRef3D + youtubeIcon + "</a>" + " " + poiText;
        }else{
            labelText = poiText;
        }

        if(infoText != "" ) {
            labelText += "" + textRef + " " + textIcon + "</a>";
        }
...
function Tooltip( mapType, paramMap, cssClassName ) {
    ...
    this.setGroundPosition_ = function (coord) {
        ...
        tooltip.setPosition(mapCoord);
        tooltip.setOffset([0, offsetY]);

        feature = null;
    };

    this.setGroundPosition = function( ){

        this.setGroundPosition_( this.coord );
    };
